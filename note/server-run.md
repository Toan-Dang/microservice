...
> Đọc kèm `note/day 2.md` (mục 3 nói về flow *sau khi* server đã chạy — tức khi có request tới; file này nói về giai đoạn *khởi động*, trước khi có request nào).
>
...
(chạy init-multiple-dbs.sh   (sẵn sàng ngay)             (Day 2 chưa dùng)           │
 ...
 auth_db/product_db/order_db)                                                        │
   ...
   │                                                                               │  │
   ▼                                                    ▼                          │  │
🟦 auth-service (gRPC :50051)                    🟩 api-gateway (HTTP :3000)        │  │
   depends_on: postgres(healthy),                depends_on: auth-service(started), │  │
               redis(healthy)                                redis(healthy)         │  │
   → xem MỤC 1 (trình tự bên trong)              → xem MỤC 2 (trình tự bên trong)   │  │
   └───────────────────────────────────────────────────────────────────────────────┘  │
                                                                                        │
   (product-service, order-service, notification-worker: CHƯA scaffold ở Day 2)         │
```

**Ý quan trọng:** `api-gateway` chỉ `depends_on: auth-service (service_started)` — tức chỉ chờ *process* auth-service bật, **không** chờ nó "sẵn sàng logic". Đây là lý do gRPC client của gateway **kết nối lười (lazy)**: nó không nối tới auth-service lúc boot, mà tới **request đầu tiên** mới nối. Nhờ vậy thứ tự bật giữa 2 service không gây crash.
...
📄 services/api-gateway/src/main.ts   ── ĐIỂM VÀO
   ...
   │           └─ controllers: 📄 auth-client/auth.controller.ts (AuthController)
   │
   │     (3) controllers của AppModule:
   │           📄 health/health.controller.ts (HealthController)
   │
   ├─[b] LIFECYCLE HOOK: onModuleInit()
   │        📄 auth-client.service.ts → onModuleInit():
   │           this.authService = client.getService('AuthService')
   │           → tạo OBJECT PROXY (có method register/login/validateToken/refreshToken)
   │           → vẫn CHƯA gửi request nào; chỉ dựng sẵn "tay cầm" để gọi sau
   │
...
- `dto/auth.dto.ts` — khi request tới, `ValidationPipe` mới dùng để kiểm body.
- `jwt-auth.guard.ts` — chỉ khi có request vào route `/auth/me`.
- `common/rpc-to-http.ts` — chỉ khi auth-service trả lỗi.
- Kết nối gRPC thật tới auth-service — chỉ xảy ra ở **request đầu tiên** (lazy connect từ mục 1 chỗ proxy).
...
t2  │ compose CHỜ healthcheck: postgres "healthy", redis "healthy"
    │
t3  │ 🟦 auth-service start:
    │     main.ts → createMicroservice → app.module.ts
    │       → ConfigModule → TypeORM connect Postgres + chạy migration (tạo bảng users)
    │       → JwtModule → RedisService connect Redis → AuthService → AuthController(@GrpcMethod)
    │       → enableShutdownHooks → listen() bind :50051  ✅ auth-service READY
    │
t3' │ 🟩 api-gateway start (song song, chỉ chờ auth-service "started"):
    │     main.ts → create → app.module.ts
    │       → ConfigModule → AuthClientModule (tạo proxy client, chưa nối)
    │         → onModuleInit dựng proxy → ValidationPipe → listen(3000)  ✅ gateway READY
    │
t4  │ Cả 2 READY. Client gọi curl :3000/... → giờ mới tới các flow ở day 2 mục 3.
    │ (Request ĐẦU TIÊN từ gateway sang auth mới thực sự mở kết nối gRPC.)
```
...
```bash
# Xem log khởi động 2 service (thấy đúng thứ tự module init + dòng "successfully started")
